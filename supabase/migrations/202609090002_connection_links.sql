begin;
alter table public.connections add column if not exists preacher_instagram varchar(2048) not null default '';
alter table public.connections add column if not exists edition_instagram varchar(2048) not null default '';
...
begin
  ...
  if existing.id is null then
    insert into public.connections(id,title,theme,event_date,preacher,preacher_title,summary,content,published,preacher_instagram,edition_instagram)
    values(target,payload->>'title',payload->>'theme',(payload->>'date')::date,payload->>'preacher',
      payload->>'role',payload->>'summary',payload->>'content',(payload->>'published')::boolean,coalesce(payload->>'preacherInstagram',''),coalesce(payload->>'editionInstagram',''));
  else
