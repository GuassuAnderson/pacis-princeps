...
import { database, PRODUCT_BUCKET } from "@/lib/server/database";
...
export async function POST(request: Request) {
  try {
    ...
    const { error: uploadError } = await db.storage.from(PRODUCT_BUCKET).upload(path, output.data, { contentType:"image/webp", cacheControl:"31536000", upsert:false });
    if (uploadError) throw uploadError;
    const { error } = await db.from("product_images").insert({ id, uploaded_by:admin.id, storage_path:path, width:output.info.width, height:output.info.height, bytes:output.info.size });
    if (error) { await db.storage.from(PRODUCT_BUCKET).remove([path]); throw error; }
    return json({ id, url:db.storage.from(PRODUCT_BUCKET).getPublicUrl(path).data.publicUrl, alt:"", width:output.info.width, height:output.info.height },201);
