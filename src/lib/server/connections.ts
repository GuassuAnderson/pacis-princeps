...
type Row = {
  ...
  preacher_title: string | null; summary: string; content: string | null; published: boolean; updated_at: string;
...
const select = 'id,title,theme,event_date,preacher,preacher_title,summary,content,published,updated_at,images:connection_assets(id,storage_path,external_url,alt,width,height,position,active)';
...
function map(row: Row): Connection {
  ...
  return { id:row.id,title:row.title,theme:row.theme,date:row.event_date,preacher:row.preacher,role:row.preacher_title || '',
    summary:row.summary,content:row.content || '',published:row.published,updatedAt:row.updated_at,images,photos:images.map(image => image.url) };
}
